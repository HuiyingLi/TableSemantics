...
import edu.cmu.lti.huiying.datastructure.Article;
import edu.cmu.lti.huiying.datastructure.Group;
import edu.cmu.lti.huiying.datastructure.Table;
import edu.cmu.lti.huiying.datastructure.Header;
import edu.cmu.lti.huiying.datastructure.Field;
...
//			}
		...
		public void characters(char ch[], int start, int length) throws SAXException {
			if(author){
				article.authors.add(new String(ch, start, length).replace("XREF", ""));
				author=false;
			}
			else if(title){
				...
				article.journalName=new String(ch,start,length).replace("XREF", "");
				journal=false;
			}
			else if(keyword){
				article.keywords.add(new String(ch,start,length).replace("XREF", ""));
				keyword=false;
			}
			else if(issn){
				article.journalISSN=new String(ch,start,length).replace("XREF", "");
				issn=false;
			}
			else if(caption){
				t.caption=new String(ch,start,length).replace("XREF", "");
				caption=false;
			}
			else if(legend){
				t.legend=new String(ch,start,length).replace("XREF", "");
				legend=false;
			}
			else if(header){
